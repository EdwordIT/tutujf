...
- (void)drawRect:(CGRect)rect {
    ...
    CGFloat originX = 0;
    if (_isHaveIcons) {
        originX = kSizeFrom750(40);
    }
    CGFloat titleX = (CGRectGetWidth(self.frame) - [self titleSize].width) * 0.5 +originX;
...
- (void)setItemTitle:(NSString *)title icon:(BOOL)isHaveIcon {
    _isHaveIcons = isHaveIcon;
    _title = title;
    [self setNeedsDisplay];
}
...
- (UIFont *)titleFont {
    UIFont *font;
    if (_selected) {
        font = [UIFont systemFontOfSize:_selectedFontSize];
    } else {
