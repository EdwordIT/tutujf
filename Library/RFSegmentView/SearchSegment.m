...
- (void)ItemStateChanged:(SegmentItem *)item index:(NSInteger)index isSelected:(BOOL)isSelected;
@end
...
- (id)initWithFrame:(CGRect)frame items:(NSArray *)items;
{
    ...
    if (self) {
        ...
        //
        self.bgView = [[UIView alloc] initWithFrame:CGRectMake(kLeftMargin, 0, viewWidth, kItemHeight)];
        ...
        if (items.count >= 2) {
            for (NSInteger i =0; i<items.count; i++) {
                SegmentItem *item = [[SegmentItem alloc] initWithFrame:CGRectMake(init_x, init_y, itemWidth, itemHeight)
...
- (void)setTintColor:(UIColor *)tintColor
{
    ...
    if (_tintColor != tintColor) {
        ...
        for (NSInteger i = 0; i<self.itemsArray.count; i++) {
            SegmentItem *item = self.itemsArray[i];
...
- (void)ItemStateChanged:(SegmentItem *)currentItem index:(NSInteger)index isSelected:(BOOL)isSelected
{
    if (self.itemsArray.count <2) {
        return;
    }
