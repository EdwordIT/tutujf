...
- (instancetype)initWithFrame:(CGRect)frame
{
    ...
    if (self) {
        _itemSize = kSizeFrom750(80);
        _lineHeight = kSizeFrom750(10);
        _minimumSize = 1;
        //底部线条
        ...
        self.backgroundLine.layer.cornerRadius = self.lineHeight/2;
...
//        self.leftCursorButton.contentHorizontalAlignment = UIControlContentHorizontalAlignmentRight;
        ...
        self.rightCursorButton.adjustsImageWhenHighlighted = NO;
...
- (void)eventPan:(UIPanGestureRecognizer*)pan
{
    ...
    CGFloat ineffectiveLength = self.itemSize;//无效的坐标系长度
    ...
    if (pan.state == UIGestureRecognizerStateEnded) {
        ...
        if(pan.view == self.leftCursorButton){
            ...
            pan.view.center = CGPointMake(countOfCalibration*widthOfCalibration+self.itemRadius, pan.view.center.y);
        }else{
            
            CGFloat countOfCalibration = round((self.controlWidth - pan.view.center.x-self.itemRadius)/widthOfCalibration);
            pan.view.center = CGPointMake(self.controlWidth - countOfCalibration*widthOfCalibration-self.itemRadius, pan.view.center.y);
        }
    }
    ...
    if(pan.view == self.leftCursorButton){
        ...
        if (CGRectGetMaxX(self.leftCursorButton.frame) > CGRectGetMinX(self.rightCursorButton.frame)) {
            ...
            frame.origin.x = CGRectGetMinX(self.rightCursorButton.frame)-widthOfCalibration;
        ...
        if (CGRectGetMinX(self.rightCursorButton.frame) < CGRectGetMaxX(self.leftCursorButton.frame)) {
            ...
            frame.origin.x = CGRectGetMinX(self.leftCursorButton.frame)+widthOfCalibration;
...
- (void)setLeftValue:(double)leftValue
{
    ...
    CGFloat widthOfCalibration = (self.controlWidth-self.itemRadius*2)/totalOfCalibration;
    ...
    frameOfLine.size.width = self.leftCursorButton.center.x - self.itemRadius;
    if(self.offsetOfAdjustLineStart){
        frameOfLine.origin.x += self.offsetOfAdjustLineStart;
        frameOfLine.size.width -= self.offsetOfAdjustLineStart;
    }
    self.leftLine.frame = frameOfLine;
}
...
- (void)setRightValue:(double)rightValue
{
    ...
    NSInteger totalOfCalibration = (self.maxValue - self.minValue)/self.minimumSize - _valueMargin;//总刻度
    CGFloat widthOfCalibration = (self.controlWidth-self.itemRadius*2)/totalOfCalibration;//份长
