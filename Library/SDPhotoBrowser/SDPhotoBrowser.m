...
- (void)photoClick:(UITapGestureRecognizer *)recognizer
{
    ...
    RFImageView * sourceView = self.sourceImagesContainerView.subviews[currentIndex];
    
    CGRect targetTemp = [self.sourceImagesContainerView convertRect:sourceView.frame toView:self];
    
    //实例化一个imageview代替之前跳转过来的view
    RFImageView *tempView = [[RFImageView alloc] init];
    ...
    [tempView setRectImageWith:currentImageView.image];
...
- (void)showFirstImage
{
    _scrollView.hidden = YES;
      ...
      RFImageView *  sourceView = self.sourceImagesContainerView.subviews[self.currentImageIndex];
    //起始位置
    CGRect startRect = [self.sourceImagesContainerView convertRect:sourceView.frame toView:self];
    RFImageView *tempView = [[RFImageView alloc] initWithFrame:startRect];
    UIImage *currentImage = sourceView.rectImage.image;
    [self addSubview:tempView];
    [tempView setRectImageWith:currentImage];

    CGFloat imageH = self.bounds.size.width*currentImage.size.height/currentImage.size.width;

    CGRect endRect = RECT(0, MAX(0,(screen_height - imageH)/2), self.bounds.size.width, imageH);
